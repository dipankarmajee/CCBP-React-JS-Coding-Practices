...
class AppStore extends Component {
  state = {searchValue: '', activeTab: tabsList[0].tabId}
  ...
  //     const getAppItem = appsList.filter(eachAppItem =>
  //       eachAppItem.appName.toLowerCase().includes(searchValue.toLowerCase()),
  //     )
  //     return getAppItem
  //   }
  ...
  filterTabList = () => {
    ...
    const filterTab = appsList.filter(
      eachCategory => eachCategory.category === activeTab,
    )
    return filterTab
  }

  activeTabSelect = value => {
    this.setState({activeTab: value})
  }

  render() {
    const {searchValue, activeTab} = this.state
    // const getAppItem = this.filterAppItem()
    const filterTab = this.filterTabList()

    const getAppItem = filterTab.filter(eachAppItem =>
      eachAppItem.appName.toLowerCase().includes(searchValue.toLowerCase()),
    )
      ...
      <div className="bg-container">
        ...
        <ul className="tab-list">
          {tabsList.map(eachTab => (
            <TabItem
              tabsList={eachTab}
              activeTabSelect={this.activeTabSelect}
              key={eachTab.tabId}
              activeTabStatus={activeTab === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="app-list">
          {getAppItem.map(eachApp => (
