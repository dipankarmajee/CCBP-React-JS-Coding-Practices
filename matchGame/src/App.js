...
//   randomImageIndex: Math.floor(Math.random() * imagesList.length),
//   score: 0,
//   timer: 3,
//   isGameRunning: true,
// }
...
class App extends Component {
  ...
  thumbnailClick = value => {
    ...
    if (imagesList[randomImageIndex].id === value) {
  ...
  render() {
    ...
    const newImageList = imagesList.filter(
      eachImage => eachImage.category === activeTabId,
    )

    const gameRunningDisplay = (
      <>
        <img
          src={imagesList[randomImageIndex].imageUrl}
          alt="match"
          className="match"
        />
        ...
        <ul className="thumbnail-container">
          {newImageList.map(eachThumbnail => (
            <li key={eachThumbnail.id}>
              <Thumbnails
                thumbnailsList={eachThumbnail}
    ...
    return (
      <div className="bg-container">
        <ul>
          <Navbar score={score} timer={timer} resetGame={this.resetGame} />
        </ul>
        <div className="game-card">
          {isGameRunning && gameRunningDisplay}
          {!isGameRunning && scoreCardDisplay}
          {/* <img
            src={imagesList[randomImageIndex].imageUrl}
            alt="match"
            className="match"
